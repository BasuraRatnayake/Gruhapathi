...
namespace Gruhapathi.Dialogs {
    public partial class DeviceAddDialog : MetroForm {
        ...
        public DeviceAddDialog(string deviceType) {
            InitializeComponent();
        ...
        private void getDevices() {
            ...
            if (deviceData.status && deviceData.response_code == 200) {
                ...
                foreach (dynamic dyn in deviceData.getDevices) {
                    if(dyn.Item4 == deviceType) {
                        cmbDoors.Items.Add(deviceType + " " + doorCount);
        ...
        private void cmbDoors_SelectedIndexChanged(object sender, EventArgs e) {
            lblDevice.Text = String.Format("Selected Device Details" + Environment.NewLine + Environment.NewLine + "Device Serial: {0}" + Environment.NewLine + "Device Version: {1}", serials[cmbDoors.SelectedIndex], versions[cmbDoors.SelectedIndex]);
        }
