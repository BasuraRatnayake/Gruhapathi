...
namespace Gruhapathi.VirtualDesign {
    public class DupWindow : Window {
        ...
        public DupWindow(Panel mainPanel,  MetroForm owner, bool power = false) : base(mainPanel, power) {
            this.owner = owner;

            deviceDiag = new DeviceDialog(owner, new Point(0, 0), Off_Click, On_Click, More_Click, "window");
        ...
        private void On_Click(object sender, MouseEventArgs e) {
            ...
            string loc = string.Format("{0},{1}", windowLocation.X, windowLocation.Y);
